...
@available(macOS 10.15, iOS 13, tvOS 13, watchOS 6, *)
public extension ObjectKeyIdentifiable {
...
public extension Publisher {
  ...
  func freeze<T>() -> Combine.Publishers.Map<Self, T> where Output: ThreadConfined, T == Output {
    return map { $0.freeze() }
  }
  ...
  func freeze<T: Object>() -> Combine.Publishers.Map<Self, ObjectChange<T>> where Output == ObjectChange<T> {
    return map {
...
public extension Publisher where Output: ThreadConfined {
  ...
  func threadSafeReference() -> Publishers.MakeThreadSafe<Self> {
...
public extension Realm {
  ...
  ///
  /// Despite the name, this actually emits *after* the collection has changed.
  var objectWillChange: Publishers.RealmWillChange {
...
public extension RealmCollection {
  /// :nodoc:
  // swiftlint:disable:next identifier_name
  func _observe<S>(on queue: DispatchQueue? = nil, _ subscriber: S)
    -> NotificationToken where S: Subscriber, S.Input == Self, S.Failure == Error
  {
  ...
  /// :nodoc:
  func _observe<S: Subscriber>(_ subscriber: S) -> NotificationToken where S.Input == Void, S.Failure == Never {
...
public enum Publishers {
  ...
  public struct Value<Subscribable: RealmSubscribable>: Publisher where Subscribable: ThreadConfined {
    ...
    init(_ subscribable: Subscribable, queue: DispatchQueue? = nil) {
  ...
  public struct ObjectChangeset<O: Object>: Publisher {
    ...
    private let queue: DispatchQueue?
    init(_ object: O, queue: DispatchQueue? = nil) {
      precondition(object.realm != nil, "Only managed objects can be published")
  ...
  public struct CollectionChangeset<Collection: RealmCollection>: Publisher {
    ...
    private let collection: Collection
    private let queue: DispatchQueue?
    init(_ collection: Collection, queue: DispatchQueue? = nil) {
  ...
  public struct DeferredHandoverCollectionChangeset<Upstream: Publisher, T: RealmCollection, S: Scheduler>: Publisher
    where Upstream.Output == RealmCollectionChange<T>
  {
    /// :nodoc:
    public typealias Failure = Upstream.Failure
    /// :nodoc:
