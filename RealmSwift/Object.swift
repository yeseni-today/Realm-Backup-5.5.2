...
/// :nodoc:
public extension _ManagedPropertyType {
  // swiftlint:disable:next identifier_name
  func _rlmProperty(_ prop: RLMProperty) {}
  // swiftlint:disable:next identifier_name
...
// MARK: - Int8 + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Int16 + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Int32 + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Int64 + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Float + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Double + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Bool + _ManagedPropertyType
/// :nodoc:
...
// MARK: - String + _ManagedPropertyType
/// :nodoc:
...
// MARK: - NSString + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Data + _ManagedPropertyType
/// :nodoc:
...
// MARK: - NSData + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Date + _ManagedPropertyType
/// :nodoc:
...
// MARK: - NSDate + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Object + _ManagedPropertyType
/// :nodoc:
...
// MARK: - List + _ManagedPropertyType
/// :nodoc:
...
// MARK: - LinkingObjects + _ManagedPropertyType
/// :nodoc:
...
// MARK: - Optional + _ManagedPropertyType
/// :nodoc:
...
// MARK: - RealmOptional + _ManagedPropertyType
/// :nodoc:
...
class ObjectUtil {
  ...
  class func getSwiftProperties(_ object: RLMObjectBase) -> [RLMProperty] {
    ...
    return getNonIgnoredMirrorChildren(for: object).compactMap { prop in
      ...
      guard let value = rawValue as? _ManagedPropertyType else {
        if class_getProperty(cls, label) != nil {
          throwRealmException(
            "Property \(cls).\(label) is declared as \(type(of: prop.value)), which is not a supported managed Object property type. If it is not supposed to be a managed property, either add it to `ignoredProperties()` or do not declare it as `@objc dynamic`. See https://realm.io/docs/swift/latest/api/Classes/Object.html for more information."
          )
        }
        if prop.value as? RealmOptionalProtocol != nil {
          throwRealmException(
            "Property \(cls).\(label) has unsupported RealmOptional type \(type(of: prop.value)). Extending RealmOptionalType with custom types is not currently supported. "
          )
        }
        return nil
      }
...
// MARK: - Object + AssistedObjectiveCBridgeable
extension Object: AssistedObjectiveCBridgeable {
  ...
  var bridged: (objectiveCValue: Any, metadata: Any?) {
    (objectiveCValue: unsafeCastToRLMObject(), metadata: nil)
  }
}
