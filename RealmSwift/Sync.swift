...
public struct SyncCredentials {
  ...
  init(_ credentials: RLMSyncCredentials) {
...
extension RLMSyncCredentials {
  convenience init(_ credentials: SyncCredentials) {
...
public extension SyncUser {
   ...
   */
  static func logIn(with credentials: SyncCredentials,
  ...
  static var all: [String: SyncUser] {
    return __allUsers()
  }
   ...
   */
  @nonobjc var errorHandler: ((SyncUser, SyncAuthError) -> Void)? {
    get {
   ...
   */
  func createOfferForRealm(at url: URL,
...
public extension SyncSession {
  ...
  struct Progress {
    ...
    init(transferred: UInt, transferrable: UInt) {
...
// MARK: - SyncPermission + RealmCollectionValue
...
public enum SyncSubscriptionState: Equatable {
  ...
  init(_ rlmSubscription: RLMSyncSubscription) {
...
public extension Results {
  ...
  func subscribe(named subscriptionName: String? = nil, limit: Int? = nil,
...
// MARK: - KeyValueObservationNotificationToken
class KeyValueObservationNotificationToken: NotificationToken {
...
public extension Realm {
  ...
  /**
    ...
    */
  func getPrivileges() -> RealmPrivileges {
   ...
   */
  func getPrivileges(_ object: Object) -> ObjectPrivileges {
   ...
   */
  func getPrivileges<T: Object>(_ cls: T.Type) -> ClassPrivileges {
   ...
   */
  func getPrivileges(forClassNamed className: String) -> ClassPrivileges {
   ...
   */
  func permissions<T: Object>(forType cls: T.Type) -> List<Permission> {
  ...
  /**
   Returns the class-wide permissions for the named class.
    ...
    - requires: This must only be called on a Realm using query-based sync.
   */
  func permissions(forClassNamed className: String) -> List<Permission> {
  ...
  /**
   ...
   `SyncSubscription.unsubscribe()` is called on them.
    ...
    - requires: This must only be called on a Realm using query-based sync.
   */
  func subscriptions() -> Results<SyncSubscription> {
  ...
  /**
   ...
   and `.error` can be retrieved with this method.
    ...
    - requires: This must only be called on a Realm using query-based sync.
   */
  func subscription(named: String) -> SyncSubscription? {
    return rlmRealm.subscription(withName: named).map(SyncSubscription.init)
  }
}

public extension List where Element == Permission {
  /**
   ...
   */
  func findOrCreate(forRoleNamed roleName: String) -> Permission {
  ...
  /**
   ...
   for the given Role, merging duplicates and inserting new ones as needed.
    ...
    - returns: A Permission object contained in this List for the named Role.
   */
  func findOrCreate(forRole role: PermissionRole) -> Permission {
    precondition(realm != nil, "Cannot be called on an unmanaged object")
